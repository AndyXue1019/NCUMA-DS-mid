...
def scan_callback(scan: LaserScan):
    ...
    if not features_to_predict:
        ...
        for obj_id in list(tracked_objects.keys()):
            ...
            if (
                tracked_objects[obj_id].time_since_update > 10
            ):  # 例如超過10幀沒看到就刪除
    ...
    # 得到機率分佈
    probabilities, _ = adaboost_predict_proba(features_scaled, stumps, alphas)
    ...
    if not tracked_objects:
        ...
        for det in current_detections:
            tracked_objects[next_object_id] = ObjectTracker(
                next_object_id, points[det['points_indices']], det['probabilities']
            )
            next_object_id += 1
    else:
        # 計算 cost matrix (距離)
        tracker_ids = list(tracked_objects.keys())
        tracker_centroids = np.array(
            [tracked_objects[tid].centroid for tid in tracker_ids]
        )
        ...
        for c in range(len(current_detections)):
            if c not in matched_detection_indices:
                ...
                tracked_objects[next_object_id] = ObjectTracker(
                    next_object_id,
                    points[detection['points_indices']],
    ...
    for obj_id, tracker in tracked_objects.items():
        ...
        if current_detections:
            dists = [
                np.linalg.norm(tracker.centroid - det['centroid'])
                for det in current_detections
            ]
            closest_det_idx = np.argmin(dists)
            if dists[closest_det_idx] < 0.5:  # 確保是同一個物體
                final_segments_to_publish.append(
                    current_detections[closest_det_idx]['points_indices']
                )
        ...
        if best_class_idx in [1, 2]:
            ...
            log_output.append(
                f'(x={obj_x:.2f}, y={obj_y:.2f}, probability={best_class_prob:.4f}, index={best_class_idx})'
            )
